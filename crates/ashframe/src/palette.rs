...
/// The mech palette. Player and hostiles have to be distinguishable at a glance
/// in a yard full of grey boxes, which is most of what the art direction is for.
pub mod mech_color {
    use godot::prelude::Color;

    /// The player: light gunmetal with a warm accent, so the eye finds it
    /// first against a grey yard. The shell is the bulk of the machine and has
    /// to be the lighter of the two; an accent that covers most of the mech is
    /// not an accent, it is a paint scheme.
    pub const PLAYER_SHELL: Color = Color::from_rgba(0.46, 0.49, 0.54, 1.0);
    pub const PLAYER_TRIM: Color = Color::from_rgba(0.84, 0.52, 0.16, 1.0);
    pub const PLAYER_GLASS: Color = Color::from_rgba(0.35, 0.75, 0.85, 1.0);

    /// Skirmisher: light, cool, fast.
    pub const SKIRMISHER_SHELL: Color = Color::from_rgba(0.40, 0.44, 0.49, 1.0);
    pub const SKIRMISHER_TRIM: Color = Color::from_rgba(0.30, 0.65, 0.78, 1.0);

    /// Artillery: heavy, ochre, deliberate.
    pub const ARTILLERY_SHELL: Color = Color::from_rgba(0.50, 0.45, 0.36, 1.0);
    pub const ARTILLERY_TRIM: Color = Color::from_rgba(0.80, 0.45, 0.15, 1.0);

    /// The Patriarch: near-black with a red core, because it should read as a
    /// different class of thing rather than a bigger skirmisher.
    pub const BOSS_SHELL: Color = Color::from_rgba(0.22, 0.22, 0.26, 1.0);
    pub const BOSS_TRIM: Color = Color::from_rgba(0.78, 0.16, 0.18, 1.0);

    pub const DARK: Color = Color::from_rgba(0.12, 0.13, 0.15, 1.0);
}

/// Proportions for one mech frame.
///
/// Everything is derived from the frame's height so an archetype is described
/// by three numbers rather than by a table of a dozen that have to agree with
/// each other. The heights come from the simulation's own config, so a mech
/// cannot be drawn at a size its collider does not have.
...
pub struct MechSpec {
    ...
    pub head_height: f32,
    pub leg_width: f32,
    /// Distance between the hip pivots, which is wider than the body: a machine
    /// this heavy stands on a base broader than the mass above it.
    pub hip_width: f32,
    /// How far the legs swing in a full stride, in radians.
    pub stride: f32,
    pub shell_color: Color,
    pub trim_color: Color,
}

impl MechSpec {
    /// The proportions for a frame, given its height, bulk and palette.
    ///
    /// `bulk` widens the machine without making it taller, which is what makes
    /// the Patriarch read as a heavier class of thing rather than as a scaled-up
    /// skirmisher.
    pub fn derive(height: f32, bulk: f32, shell_color: Color, trim_color: Color) -> Self {
        Self {
            height,
            // Just over half the machine is leg. A mech with a short stance and
            // a deep chest reads as a toy; the proportions that read as a
            // machine are the ones a walking vehicle would actually need —
            // enough leg to clear obstacles, and a body narrow enough that the
            // legs are visible either side of it.
            hip_height: height * 0.52,
            body_width: height * 0.26 * bulk,
            body_depth: height * 0.20 * bulk,
            chest_height: height * 0.22,
            waist_height: height * 0.055,
            head_height: height * 0.095,
            leg_width: height * 0.095 * bulk,
            // Wider than the body, so the stance is legible from any angle.
            hip_width: height * 0.30 * bulk,
            // A heavier machine takes shorter steps; the stride is what carries
            // the sense of weight at a fixed walking speed.
            stride: 0.55 / bulk,
            shell_color,
            trim_color,
        }
    ...
    pub fn for_frame(frame: crate::rig::Frame) -> Self {
        ...
        match frame {
            crate::rig::Frame::Player => Self::derive(
                player::HEIGHT,
                1.0,
                mech_color::PLAYER_SHELL,
                mech_color::PLAYER_TRIM,
            ),
            crate::rig::Frame::Hostile(EnemyKind::Skirmisher) => Self::derive(
                skirmisher::HEIGHT,
                0.9,
                mech_color::SKIRMISHER_SHELL,
                mech_color::SKIRMISHER_TRIM,
            ),
            crate::rig::Frame::Hostile(EnemyKind::Artillery) => Self::derive(
                artillery::HEIGHT,
                1.05,
                mech_color::ARTILLERY_SHELL,
                mech_color::ARTILLERY_TRIM,
            ),
            crate::rig::Frame::Hostile(EnemyKind::Boss) => Self::derive(
                boss::HEIGHT,
                1.25,
                mech_color::BOSS_SHELL,
                mech_color::BOSS_TRIM,
            ),
        }
    }
}
